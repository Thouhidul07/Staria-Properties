        ...
        <div className="max-w-[1440px] mx-auto px-12 xl:px-20">
          ...
          <motion.div key={projectFilter} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <AnimatePresence mode="wait">
              {PROJECTS.filter((p) => projectFilter === "All" || p.category === projectFilter).map((project, i) => (
                <motion.div key={project.id} initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }} transition={{ duration: 0.6, delay: i * 0.08, ease: [0.22, 1, 0.36, 1] }}
                  ...
                  <div className="px-5 py-5">
                    ...
                    <button className="w-full py-3 border border-[#0B5E3C]/25 hover:bg-[#0B5E3C] hover:border-[#0B5E3C] text-[#0B5E3C] hover:text-white text-[1rem] font-semibold tracking-[0.02em] rounded-xl flex items-center justify-center gap-2 transition-all duration-400" style={{ fontFamily: "'DM Sans', sans-serif" }}>
